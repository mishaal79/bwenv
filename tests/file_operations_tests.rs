...
"#;
    ...
    assert_eq!(env_vars.get("API_KEY"), Some(&"sk_test_abcdefghijklmnop".to_string()));
...
"#;
    ...
    assert_eq!(merged_vars.get("ANOTHER_BASE"), Some(&"base_data".to_string()));
    assert_eq!(merged_vars.get("NEW_KEY"), Some(&"new_value".to_string()));
    assert_eq!(merged_vars.get("ANOTHER_NEW"), Some(&"more_data".to_string()));
    
    // Shared key should be updated
    assert_eq!(merged_vars.get("SHARED_KEY"), Some(&"updated_value".to_string()));
...
fn test_overwrite_vs_merge_behavior() {
    ...
    assert_eq!(overwrite_result.get("NEW_KEY"), Some(&"new_value".to_string()));
    assert_eq!(overwrite_result.get("SHARED_KEY"), Some(&"new_value".to_string()));
    ...
    assert_eq!(merge_result.get("INITIAL_KEY"), Some(&"initial_value".to_string()));
    assert_eq!(merge_result.get("NEW_KEY"), Some(&"new_value".to_string()));
    assert_eq!(merge_result.get("SHARED_KEY"), Some(&"new_value".to_string()));
}
...
"#;
    ...
    assert!(result1.unwrap_err().to_string().contains("missing '=' character"));
...
"#;
    ...
    assert_eq!(env_vars.get("URL_WITH_PROTOCOL"), Some(&"https://example.com:8080/path?query=value&other=data".to_string()));
    assert_eq!(env_vars.get("JSON_CONFIG"), Some(&r#"{"key":"value","number":123,"boolean":true}"#.to_string()));
    assert_eq!(env_vars.get("PATH_WITH_SPACES"), Some(&"/path/with spaces/file.txt".to_string()));
    assert_eq!(env_vars.get("SPECIAL_CHARS"), Some(&"!@#$%^&*()_+-={}[]|;:,.<>?".to_string()));
...
"#;
    ...
    assert_eq!(env_vars.get("KEY_WITH_MULTIPLE_EQUALS"), Some(&"value=with=equals=signs".to_string()));
    assert_eq!(env_vars.get("KEY_WITH_LEADING_SPACES"), Some(&"value_with_spaces".to_string()));
    assert_eq!(env_vars.get("KEY_WITH_LEADING_SPACES2"), Some(&"value2".to_string()));
    assert_eq!(env_vars.get("KEY_EMPTY_VALUE"), Some(&"".to_string()));
    assert_eq!(env_vars.get("KEY_ONLY_SPACES"), Some(&"".to_string()));
    assert_eq!(env_vars.get("KEY_WITH_HASH_IN_VALUE"), Some(&"value#with#hash".to_string()));
    assert_eq!(env_vars.get("KEY_WITH_QUOTE"), Some(&"value\"with\"quotes".to_string()));
    assert_eq!(env_vars.get("KEY_WITH_NEWLINE"), Some(&"value\\nwith\\nnewlines".to_string()));
}
...
fn test_write_env_file_creates_readable_format() {
    ...
    let var_lines: Vec<&str> = lines.iter()
...
fn test_file_permissions_and_access() {
    ...
    assert_eq!(overwritten_vars.get("TEST_KEY"), Some(&"test_value".to_string()));
    assert_eq!(overwritten_vars.get("NEW_KEY"), Some(&"new_value".to_string()));
}